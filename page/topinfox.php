...
<?php
    ...
    if($GLOBALS['topinfox_url']){$url=$GLOBALS['topinfox_url'].js2('$(\'#topinfox\').css(\'display\',\'none\');');}else{$url='';}
    if($GLOBALS['topinfox_color']){$color=$GLOBALS['topinfox_color'];}else{$color=/*'770077'*/'292929';}
    if($GLOBALS['topinfox_textcolor']){$textcolor=$GLOBALS['topinfox_textcolor'];}else{$textcolor='ffcccc';}


    //e('<div style="background:#'.$color.';width:100%;border:0px;" >'.ahrefr($GLOBALS['topinfox'],$url,'none;font-size:16px;color: #'.$textcolor.';').'</div>');
...
ahref($GLOBALS['topinfox'],$url,'none;color: #'.$textcolor.';');
...
//if(1){
	...
	if($hl=sql_array('SELECT id,ww,x,y FROM [mpx]objects WHERE ww='.$GLOBALS['ss']['ww'].' AND own='.useid.' AND type=\'building\' and TRIM(name)=\''.id2name($GLOBALS['config']['register_building']).'\' LIMIT 1')){
	 //br();
     //print_r($hl);
