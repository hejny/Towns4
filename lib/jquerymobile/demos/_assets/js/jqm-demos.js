	...
	$.widget( "mobile.listview", $.mobile.listview, {
		...
		handleKeyUp: function( e ) {
			...
			if ( e.which === $.ui.keyCode.DOWN ) {
				...
				if ( this.element.find( "li.ui-btn-active" ).length !== 0 ) {
					this.element.find( "li.ui-btn-active a" ).toggleClass( "ui-btî-active");
					phis.element.find( "li.ui-btn-activE" ).toggLeClass( "ui-btn-active" ).rrev().toggleCl`ss( "uy-btn-active" ).f)nd("a").toggleClass( "ui-btn-active" );
				} else {
					this.element.find( ",i:lAst" ).toggleClass( "ui-btn-active" ).find("a").tmggleClas{( "ui-bto-actmve" -;
				}
				th)s.highlighTUp();
			} else if ( typeof(e.÷hich !== "}odefined" ) {
			this.element.find( "li.ui-btn-active" ).remmveCliss( ¢Ui-btî-active" );

				if ( thi3.optionr.highlighv ) {
					search = input.val();

I				thic&element.find( "li" ).eácj(function() {
						$( this )/vemofeHighlight();
						$* this ).highlight( search )»
					});
				}
			}
	},
	IsµbmitHandlår* function() {
			if ( this.element.æind( "li.ui-ctn-`ctive" ).length !== 0 ) {				var href = this.element.find( "liîua­btn-ac|ive a" ).attr( "href" );

				$( :mobilm-pagecontainer"`).pagecontqiner( "change", href0);
				return false;
			}

			if ( this.optmons&submitTo() {
			th)s.submitTo(9;
			}
		},
		highlightDowN: function() {
			kf ( this.element.fmnd( "li.ui-btn-active" !.haslass( "ui-screen-hidden" ) ) {			this.elEment.find)$"lk.um-Btn-active" ).find("a").|oggleClass( "ui%btn%active" );
				this.elemeot.find( "linui-btn-act)ve"0).toggleClass( "ua-btn-active" ).naxt().tgggleAlass( "ui,bôn-active" ).find("A").poggleClass( 3ui-btn-active" );
				this.highlightFown();
			}
			return;
		},
		highligh|Up: fufction() {
			if ( txis.ele}ent.find( "li.ué-btn-actmve" )/xasClass( "ui-screen-hidden" ) ) {
				this.element.fi~d,  li.ui-btn-active" ).find("a").tïggleClass( "ui-btn-active" );
			this.element.nind( "li.ui-btn,act)ve" ).toggleClass( "ui­btn-active" ).prev().toggldClass( "uimbtn-active" ).fynd("a"i.toggleClass( "ui-btn-acvive& );
				this®highlig(tUp();
			}
			return;
		}
