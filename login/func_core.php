...
//------------------------------------------------------------------------
    ...
    if($param1='new' or !($error=name_error($param1))){
		...
		$randomcolor='';//rand_color();
                ...
                sql_query/*e*/("INSERT INTO ".mpx."objects (`id`,`name` ,$rows, `profile`, `set`, `own`, `in`, `ww`, `x`, `y`, `t`) SELECT '$id','$param1',$rows, 'color=$randomcolor','$set', '0', '0', '".$GLOBALS['ss']["ww"]."', '0', '0', '".time()."' FROM ".mpx."objects WHERE id='".register_user."';");
                $id2=nextid();                
                sql_query/*e*/("INSERT INTO ".mpx."objects (`id`,`name` ,$rows, `profile`, `set`, `own`, `in`, `ww`, `x`, `y`, `t`) SELECT '$id2','$param1',$rows, 'color=$randomcolor','coolround=".time()."', '$id', '0', '".$GLOBALS['ss']["ww"]."', '$x', '$y', '".time()."' FROM ".mpx."objects WHERE id='".register_town."';");
                $id3=nextid();                
                sql_query/*e*/("INSERT INTO ".mpx."objects (`id`,`name` ,$rows, `profile`, `set`, `own`, `in`, `ww`, `x`, `y`, `t`) SELECT '$id3',`name`,$rows, `profile`,'', '$id2', '0', '".$GLOBALS['ss']["ww"]."', '$x', '$y', '".time()."' FROM ".mpx."objects WHERE id='".register_building."';");
		if(defined('register_flag')){
			...
			$rows='`type`, `dev`, `fr`, `fx`, `fc`, `func`, `hold`, `res`, `profile`, `hard`, `expand`';              
		        sql_query/*e*/("INSERT INTO ".mpx."objects (`id`,`name` ,$rows, `fs`, `fp`, `set`, `own`, `in`, `ww`, `x`, `y`, `t`) SELECT '$id4',`name`,$rows, `fs`/5, `fp`/5,'', '0', '0', '".$GLOBALS['ss']["ww"]."', '$xf', '$yf', '".time()."' FROM ".mpx."objects WHERE id='".register_flag."';");
			//die();
		}
...
function a_login($param1,$param2='towns',$param3='',$param4='',$param5=''){
    ...
    if($param2=='towns'){
        ...
        if($pass==md5($param3) or !$pass){
            ...
            //echo("abc");
            $use=sql_1data('SELECT `id` FROM [mpx]objects WHERE `own`=\''.($GLOBALS['ss']["log_object"]->id).'\' AND `type`=\'town\' ORDER BY ABS('.($GLOBALS['ss']["log_object"]->id).'-`id`) LIMIT 1');
            //die($use);            
            if($use){
        ...
        if(1==sql_query('UPDATE [mpx]login SET time_use = \''.time().'\' WHERE `id`=\''.($param1).'\' AND `method`=\'facebook\' AND `key`=\''.($param3).'\' ')){      
            ...
            $use=sql_1data('SELECT `id` FROM [mpx]objects WHERE `own`=\''.($GLOBALS['ss']["log_object"]->id).'\' AND (`type`=\'town\' OR `type`=\'town2\') ORDER BY ABS('.($GLOBALS['ss']["log_object"]->id).'-`id`) LIMIT 1');
            //die($use);            
            if($use){
...
function force_login($param1){
	$param1=sql($param1);
        $GLOBALS['ss']["log_object"] = new object(NULL,"type='user' AND (id='$param1' OR name='$param1')");
        $use=sql_1data('SELECT `id` FROM [mpx]objects WHERE `own`=\''.($GLOBALS['ss']["log_object"]->id).'\' AND (`type`=\'town\' OR `type`=\'town2\')');        
        $GLOBALS['ss']["logid"]=$GLOBALS['ss']["log_object"]->id;
        a_use($use);     
}
//======================================================================================LOGOUT
