...
<form action="?page=botx" method="POST"><input type="text" name="create" size="100" value=""><input type="submit" value="vytvořit">
<?php
//$where="type='user' AND ww!=0 AND ww!=-1 AND 'f561aaf6ef0bf14d4208bb46a4ccb3ad'=(SELECT `key` FROM [mpx]login as x WHERE x.id=`[mpx]pos_obj`.id AND `method`='towns' LIMIT 1) "
...
e('</tr>');
	...
	list($id,$name,$type,$fs,$fp,$fr,$fx,$own,$in,$x,$y,$t,$ww,$ad1,$ad2,$ad3)=$row;
   	$lvl=fs2lvl($ad2);
         e('<tr>');
	   e('<td>'.$id.'</td>');
	   e('<td>'.$name.'</td>');
	   ...
	   e('<td>'.$ad1.'</td>');
	   e('<td>'.$lvl.'</td>');
            e('<td><a href="?page=users&amp;login='.$id.'" target="_blank">přihlásit se</a></td>');
   e('</tr>');
}
e('</table>');
?>
