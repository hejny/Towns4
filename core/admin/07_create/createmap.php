...
$imgurl=adminfile.'files/'.$GLOBALS['ss']["image"].'.png';
...
Image: <?php echo(adminfile); ?>files/<input name="image" type="text" id="image" value="<?php echo($GLOBALS['ss']["image"]); ?>" />.png<br>
...
//-------
if($action=="tables"){
	$sql=file_get_contents(adminfile.'sql/create_map.sql');
	$sql=str_replace('[mpx]',mpx,$sql);
	sql_query($sql);
	$sql=file_get_contents(adminfile.'sql/create_objects.sql');
	$sql=str_replace('[mpx]',mpx,$sql);
	sql_query($sql);
	$sql=file_get_contents(adminfile.'sql/create_text.sql');
	$sql=str_replace('[mpx]',mpx,$sql);
	sql_query($sql);
	$sql=file_get_contents(adminfile.'sql/create_memory.sql');
	$sql=str_replace('[mpx]',mpx,$sql);
	sql_query($sql);
	$sql=file_get_contents(adminfile.'sql/create_login.sql');
	$sql=str_replace('[mpx]',mpx,$sql);
	sql_query($sql);
}

if($action=="treeS"){
    $file=adminfile."objects/tree.txt";
    echo($file." saved");
    file_put_contents($file,serialize($post));
    chmod($file,0777);
}
if($action=="rockS"){
    $file=adminfile."objects/rock.txt";
    echo($file." saved");
    file_put_contents($file,serialize($post));
    chmod($file,0777);
}
...
<form id="form1" name="form1" method="post" action="?">
  <table width="800" border="0">
    <tr>
      ...
      <img src="<?php echo(adminfile."model.php?model="."&rotation=".$rot."&sun=1&size=1"); ?>" />
      </td>
    </tr>
    <tr>
...
if($action=="map" or $action=="tree" or $action=="rock" or $action=="finish"){echo("<br>mapgenerator<br>");
	...
	$hold=("fuel=".rand($post['fuel'],$post['fuelx']).";wood=".rand($post['wood'],$post['woodx']).";stone=".rand($post['stone'],$post['stonex']).";iron=".rand($post['iron'],$post['ironx']).";");
            ...
            //------------------
	//sql_query("INSERT INTO `[mpx]pos_obj` (`name`, `type`, `fs`, `fp`, `fr`, `fx`, `fc`, `func`, `hold`, `res`, `profile`, `set`, `hard`, `own`, `in`, `ww`, `x`, `y`, `t`) VALUES ('{rock} [$x,$y]', 'rock', '".pow($defence,2)."', '".pow($defence,2)."', '".($a+$b)."', '".($a+$b+pow($defence,2))."', 'fp=0;iron=".ceil(pow($defence,2)*(2/3)).";fuel=".ceil(pow($defence,2)*(1/3))."', 'defence=class[5]defence[3]params[5]defence[7]5[10]$defence"."[7]2[10]1[3]0[5]profile', '".$hold."', '$res', '', '', '1', '0', '0', '".$GLOBALS['ss']["ww"]."', '$xx', '$yy', '".time()."')");
