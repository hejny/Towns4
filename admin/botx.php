...
$zone=$_POST['zone'];
   ...
   $keys='`id`, `method`, `key`, `text`, `time_create`, `time_change`, `time_use` ';
   sql_query("UPDATE [mpx]objects SET name='$name' WHERE id=".$logid,2);br();
   $pass=md5(rand(1111,9999));
   sql_query("INSERT INTO [mpx]login ($keys) VALUES (".$logid.",'bot','".$pass."','".($zone)."',".time().",".time().",0)",2);br();
   sql_query("INSERT INTO [mpx]login VALUES (".$logid.",'towns','".$pass."','bot',".time().",".time().",0)",2);br();
   //$zone=rand(3600,3600*24);
   }}
}
...
<input type="text" name="zone" value="<?php e(rand(20,80)); ?>">
...
$ad5='SELECT text FROM [mpx]login as x WHERE x.id=[mpx]objects.id AND x.method=\'bot\'';
$ad0=',('.$ad0.') as ad0';
$ad1=',('.$ad1.') as ad1';
$ad2=',('.$ad2.') as ad2';
$ad3=',('.$ad3.') as ad3';
...
$array=sql_array("SELECT `id`,`name`,`type`,`dev`,`fs`,`fp`,`fr`,`fx`,`own`,`in`,`x`,`y`$ad0,`ww`$ad1$ad2$ad3$ad4$ad5 FROM `".mpx."objects` WHERE ".$where." ORDER BY $order");
...
foreach($array as $row){
   list($id,$name,$type,$dev,$fs,$fp,$fr,$fx,$own,$in,$x,$y,$t,$ww,$ad1,$ad2,$ad3,$ad4,$ad5)=$row;
   $lvl=fs2lvl($ad2);   
