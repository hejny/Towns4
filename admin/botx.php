...
if($_GET['delete']){   deleteuser($_GET['delete'])
   ...
   xquery('register',$name,$GLOBALS['inc']['bot_password'],str_replace('[name]',$name,$GLOBALS['inc']['bot_email']),0)
	sql_query('UPDATE [mpx]objects SET type=\'town2\' WHERE id='.$GLOBALS['ss']['useid'])
	xreport()
   }}}?><form action="?page=botx" method="POST"><input type="text" name="create" size="100" value=""><input type="submit" value="vytvořit"><?php//$where="type='user' AND ww!=0 AND ww!=-1 AND 'f561aaf6ef0bf14d4208bb46a4ccb3ad'=(SELECT `key` FROM [mpx]login as x WHERE x.id=[mpx]objects.id AND `method`='towns' LIMIT 1) "
$where="type='user' AND ww!=0 AND ww!=-1 AND 1=(SELECT 1 FROM [mpx]users as x WHERE x.password='".md5($GLOBALS['inc']['bot_password'])."' AND x.id=`".mpx."objects`.userid LIMIT 1) "
...
$ad3=',('.$ad3.') as ad3'
$order=" ad2 DESC"
$array=sql_array("SELECT `id`,`name`,`type`,`fs`,`fp`,`fr`,`fx`,`own`,`in`,`x`,`y`$ad0,`ww`$ad1$ad2$ad3 FROM `".mpx."objects` WHERE ".$where." ORDER BY $order")
